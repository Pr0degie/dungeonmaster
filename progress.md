...
- _Aufgeschoben: **E4** (`StreamAssembler`+`finalize_answer` → `llm/stream_assembler.py`) und die **`dmcog.py`-Splits**
  (Lore-Cog nach `_speak`/`_synthesize`→Runtime; Scene-Mixin) — Letztere brauchen ein neues Mixin-Idiom + eigene ADR._
