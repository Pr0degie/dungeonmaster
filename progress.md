...
- **Newest round:** Spielerdoku [docs/characters.html](docs/characters.html) (2026-08-22, Akten aller
  ...
  den Enginseer Vinzentius Kabelbrand, der die zwei Lücken füllt (Fernkampf, Technologie); der
...
- **Open live gates (10, all stacked into the one live run):** augmetics (D52 — rested until
  a player carried an implant; Vinzentius carries three), NPC memory (D91), consistency guard (D92), clocks (D94), in-game time/deadlines (D95), NPC agendas (D96), Chekhov threads
...
| D105 | Vincent kommt als vierter Spieler dazu; die Party hatte weder Fernkampf noch Technologie, und drei Run-Dokumente nannten sie namentlich mit drei Namen | Enginseer Vinzentius Kabelbrand, profilgeprüft gebaut und in BEIDE geladenen `characters.json` (Channel + `_default`) einsortiert; Run-Doku nachgezogen, Augmetik-Gate D52 wieder aufgenommen (Wahrnehmung 44 statt 39) | Kein Trade-off, daher kein ADR: Inhalt + Doku-Sync. Die `_default`-Datei ist der Fallback auf einem fremden Server — nur die Channel-Datei zu pflegen hätte den Spieler dort stumm verschluckt |
