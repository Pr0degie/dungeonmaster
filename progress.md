...
Text** klingen. Harte Randbedingung: XTTS ~**0,5× Echtzeit** → sofortiger Start UND lückenlos geht nicht gleichzeitig;
...
- **+3 Tests** (`tests/test_silent_wav.py`): Reihenfolge+Gap, Null-Gap = reine Konkatenation, Einzelteil ohne
  Trailing-Gap. Suite **309 grün**; `dmcog`-Import im venv geprüft. _Live offen: `!intro test` anhören (durchgehend?
  `first_audio`-Wartezeit erträglich?); Trade-off-Hebel = Länge kürzen oder Streaming-`!intro` (mit Lücken)._

_Ältere `## Last session`-Einträge (D64 2000-Zeichen-Fix, D63 Kontext-Split u. a.): siehe **[docs/progress-archive.md](docs/progress-archive.md)**._
