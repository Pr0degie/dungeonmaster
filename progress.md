...
- **Newest round:** Playtest-Triage nach dem ersten Debug-Abend (2026-08-15, D103 → ADR 056):
  Er lief in der falschen Kampagne, weil die ADR-055-Sandbox nur Archive und RAG-Source
  trennte. Debug-Runs bekommen jetzt `.debug`-Zwillinge für state/history/chekhov/recap, ein
  fremder Szenenzeiger fällt beim `!j` laut auf die Startszene zurück, und „Als Spielleitung
  antworte ich" wird gestrippt. Suite 763 grün, `dm-eval` Exit 0.
- **Next concrete step:** Die Debug-Kampagne erneut live spielen
  ([docs/debug-campaign-runbook.md](docs/debug-campaign-runbook.md)) — dieselben Gates, jetzt
  mit sauberem Start; alternativ [das One-Shot-Skript](docs/live-run-script.md).
- **Open live gates (9, all stacked into the one live run):** NPC memory (D91), consistency
  guard (D92), clocks (D94), in-game time/deadlines (D95), NPC agendas (D96), Chekhov threads
  (D97 — needs a 2nd session), campaign memory (G10/D102 — rides that same 2nd session)
  + Phase 9 (HP survives restart, recap) and Phase 10 half 1 (rule question from RAG).
  Over the WIP limit → next feature round that would open a new gate must wait for the
  live run; doc/tooling rounds are exempt.
...
_Ältere Current-focus-Blöcke (Session-RAG-Runde D101 → ADR 054, Journal-Runde D100 → ADR 053, Content-Runde Debug-Kampagne „Die Mitternachtsfracht“ 2026-07-11, D99 Debug-Overlay, D98 Marker-Registry u. a.): siehe [docs/progress-archive.md](docs/progress-archive.md), `## Current focus (Verlauf)`._
...
- **Nicht in dieser Runde, aus demselben Log:** Timos `.env` (3 unparsbare Zeilen → Keys stumm ignoriert; `TTS_SPEAKER=cuda` statt eines Speaker-Namens), englische Einsprengsel in der Narration („Schritte outside der Tür"), `DM_NUM_PREDICT=220` als faktische Turn-Länge (885 Zeichen = 66 s Wiedergabe, 78 % der Gesamtlatenz), und die durch den Fehlstart in die Live-Dateien geschriebenen Turns dieses Abends.
