...
- **Open live gates (10, all stacked into the one live run):** augmetics (D52 — rested until
  ...
  D52 rides the scheduled run. **WIP-Override (2026-08-23, D107):** die Spezifikationsrunde
  eröffnet sieben weitere Gates — alle liegen auf demselben Abend, ohne die Fixes wäre er
  wieder unbrauchbar.
...
| D107 | Der Debug-Abend blieb 22 Züge in Szene eins — der Szenenwechsel hing allein an einem Inline-Marker, der nie feuerte | Szenenwechsel wird Code-Sache: Klassifikator über die echten Ausgänge der Szene plus Flag-Zwang bei erschöpfter Szene, automatisch mit einminütigem Undo | Pflichtentscheidungen gehören nicht an einen Inline-Marker, der am gekappten Textende steht → **ADR 057** |
| D108 | Eine ausgehändigte Zollvollmacht existierte neun Züge später nicht mehr | Zweiter Klassifikator auf derselben Zugnaht schreibt Übergaben, angenommene Aufträge und Versprechen als harte Fakten in den Weltzustand | Erzähltext ist kein Gedächtnis; das Modell fragt, Code validiert und schreibt → **ADR 058** |
| D109 | Die Ingame-Uhr stand den ganzen Abend still, obwohl die Kampagne auf einer Mitternachtsfrist steht | Startzeit, Fristen und Uhren kommen aus dem Abenteuer; Zeit läuft pro Zug in Code, `<<ZEIT>>` bleibt für bewusste Sprünge | Zwei fertige ADRs (047/048) liefen nie live, weil ihr einziger Auslöser fehlte → **ADR 059** |
| D110 | Antwortlänge, Barge-in und ein Puppeting-Filter standen zur Debatte | Länge bleibt (ausdrücklich gewollt), Barge-in fällt weg (bräuchte Bot As Repo), Puppeting-Filter aufgeschoben: erst den Prompt-Widerspruch beheben, dann im nächsten Lauf zählen | Bewusste Nicht-Entscheidungen, festgehalten damit sie nicht als vergessen gelten |
| D111 | Reden mehrere Spieler in einem Zug, klassifizierte der Würfel-Router nur die LETZTE Pufferzeile — jede andere Ansage verschwand ohne Logzeile | Eine Aktion pro Sprecher (`llm/turn_actions.group_by_speaker`), der Router geht sie durch und postet einen Würfelknopf je ansagender Figur; die Figur kommt weiter vom Sprecher, nie vom Klickenden | Bewusst sequentiell statt `gather`: der Router legt sein Rohverdikt in EINEM `last_router`-Feld ab, parallele Aufrufe würden das Replay-Journal falsch zuordnen |
...
- **Deterministischer Meta-Satz-Filter** — „Es tut mir leid, aber ich kann Ihre Frage nicht
  verstehen" und „In diesem Fall würde ich als Spielleitung antworten:" wurden vorgelesen; es gibt
  keinen Filter dafür. Offen, weil unklar ist, ob der geschärfte Prompt reicht.
- **Halluzinations-Blockliste für STT** — Tobi wollte „Das war's für heute, tschüss" blocken;
  `segments.py` hat gar keine Blockliste, nur Wahrscheinlichkeitsschwellen. Eine zu bauen ist eine
  eigene Entscheidung (fest verdrahtet, konfigurierbar, oder nur der Abspann-Sonderfall?).
- **Identität am Würfelknopf** — ✅ erledigt in D111. Der Knopf bleibt bewusst anonym (jeder darf
  ihn drücken, damit ein abwesender Spieler den Tisch nicht blockiert), aber die Figur kommt vom
  Sprecher der Ansage, und seit D111 bekommt JEDE ansagende Figur ihren eigenen Knopf.
  Anmerkung zur Erstanalyse: der Befund „dreimal würfelt Gellicus, egal wer sprach" war falsch —
  Timo spielt Gellicus, alle drei Proben am 22.08. waren korrekt zugeordnet.
- **Stall-Detektor** — nach N Zügen ohne Fortschritt selbst weiterschieben. Aufgeschoben, weil drei
  Beweger des Szenenzeigers im ersten Lauf schwer auseinanderzuhalten wären (ADR 057).
- **Sprechmodus-Default** — bleibt `stream`+`flach`, bis die drei Modi am Tisch verglichen sind.
