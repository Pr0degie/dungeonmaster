...
| D92 | Consistency guard: how to stop a dead/absent NPC "speaking" without an LLM judge? | **Deterministic pre-delivery check (`dmbot/llm/consistency.py`), regenerate once, strictly fail-open.** `check(text, world_state, scene)` flags speech *attribution* (never mere mention) to a dead (`wounds<=0`) or scene-absent registered NPC — deliberately conservative heuristics because a false positive costs a full regeneration: present-tense speech verbs only (Präteritum = memories/recaps, allowed), paired quote spans blanked before matching (recounting doesn't flag), indefinite references („ein Kultist ruft") skipped, name tokens shared across NPC/PC names ambiguous → dropped. On a violation `DMBrain.respond`/`redo` regenerate ONCE with a concrete German `KORREKTUR:` nudge (echo/intro-nudge mechanism, marker queues snapshot/cleared/restored like `redo`); a still-violating retry is delivered anyway + warn log. The **streaming path only logs** (full text exists only after the audio played — trade-off in the ADR); the regenerate protection is batch-path (`nahtlos`, dice-consequence turns). `DM_CONSISTENCY_GUARD=1` kill switch. Suite **516 green** (+30) | LLM-judge, location/inventory checks, multi-retry and mid-stream abort all rejected (latency, false-positive multiplication, table experience) — details + live gate (provoke a dead NPC, watch the regenerate) in **→ ADR 045** |
| D93 | Replay-eval harness: how to prove a refactor didn't change pipeline behaviour? | **Golden transcripts + playback replay (`uv run dm-eval`), regression not quality.** The `history.jsonl` autosave doubles as a backward-compatible **replay journal**: a `{"kind":"session"}` header per `!join` plus optional per-turn fields (structured `lines`/`results`, the kept answer's **raw** LLM text with markers, the queued `markers`, the router's raw JSON + parsed decision, `state_before` + scene/flag verdicts — `load_recent` ignores all of it). `dmbot/tools/eval_replay.py` replays trimmed journals through a fresh `DMBrain` with a `PlaybackClient` (recorded raw responses in call order) and diffs per turn in six categories (`turn`/`answer`/`marker`/`router`/`state`/`llm`), one `[eval] DIFF` line each, exit 0/1/2 — a merge gate for refactor rounds. The flag rule moved into the pure `delivery.erledigt_verdict` (one source for cog + harness). 2 synthetic goldens in `tests/golden/` (generated through the real capture path; `generate_synthetic.py` = the bless step) + README for pulling live goldens. Suite **535 green** (+19) | Recorded-live expectations pin what the table actually saw (bless-on-first-run would pin possibly-broken code); full state-snapshot diffs rejected — dice-click RNG/GM commands/NPC-memory writes land asynchronously and would drown the diff in unreplayable noise, so the **verdicts** are compared instead; dice-value recording + engine re-run deferred (dense fixed-seed unit tests already cover it). Live model comparison (Nemo vs. Mistral Small) is the explicit follow-up round on this harness → **ADR 046** |
