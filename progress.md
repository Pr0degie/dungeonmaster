...
- **Newest round:** Debug-Sandbox + G10 (2026-07-17, D102 → ADR 055): Debug-Runs rotieren
  als `history.<stamp>.debug.jsonl` und routen per Dateiname in die separate Source
  `session_debug_<channel_id>` — Kreuz-Kontamination mit dem Live-Kampagnengedächtnis
  strukturell unmöglich, Reset per `--wipe-debug`. G10 (Kampagnen-Gedächtnis) reitet auf
  der G9-Zweitsession. Suite 754 grün, ruff sauber, `dm-eval` Exit 0.
...
pro Gate; Chekhov (G9) = Session-1-Saat / Session-2-Ernte. `docs/live-test-checklist.md`
ist darin aufgegangen (Redirect-Stub); der frühere Gate-Fahrplan dieses Abschnitts liegt
verbatim im Archiv (`## Next concrete step (Verlauf)`).
...
- ✅ Aufräum-Pass `progress.md` erledigt (D98 Teil B + Doc-Diet-Runde) — Details im Archiv.
...
- ✅ Kosmetik geputzt (D98, `logsetup`-Docstring) — Details im Archiv.
