...
- **Newest round:** Session-RAG (2026-07-17, D101 → ADR 054): Kampagnengedächtnis — rotierte
  Journale pro Szene in den Store (`!leave`-Ingest + `!join`-Catch-up), hybrides KNN+FTS-
  Retrieval, max 2 Chunks nach Weltwissen, `DM_SESSION_MEMORY`-Knopf. Suite 748 grün,
  `dm-eval` Exit 0, Kalibrierung 5/5 (FTS-getragen). Kein neues Live-Gate; das
  `SESSION_MAX_DISTANCE`-Livetuning reitet auf dem ohnehin anstehenden Live-Run mit.
...
_Ältere Current-focus-Blöcke (Content-Runde Debug-Kampagne „Die Mitternachtsfracht“ 2026-07-11, D99 Debug-Overlay, D98 Marker-Registry u. a.): siehe [docs/progress-archive.md](docs/progress-archive.md), `## Current focus (Verlauf)`._
