...
## Next concrete step
**0. Vorab, 2 Minuten (D89):** `uv run python tools/sync_check.py` — die 20 fehlenden `.env`-Keys aus `.env.example` nachziehen (funktional gleich [Code-Defaults], aber die Tuning-Knöpfe `DM_REPEAT_PENALTY`/`DM_INTRO_TEMPERATURE` müssen zum Live-Drehen in der `.env` stehen). Vor dem nächsten Timo-Sync: beide Maschinen `sync_check`, Blöcke diffen (SETUP.md §„Staying in sync").
