...
- **#3 verworfen** (Tobi: Turn-Reihenfolge egal — jeder Spieler gibt Input, das LLM treibt die Geschichte). **#4/#5/#6** als „auch gut, später" geparkt (siehe `## Next concrete step`).
...
_Ältere `## Last session`-Einträge (D77 Dev-Gates [Lint-Stop-Hook + blockierender git-pre-commit + Review/Simplify-Checkliste], D76 `disconnect_voice`-Kontrakt + neuer Delivery-Test, D75 One-Shot-Setup, D74
...
5. **`/improve-architecture`-Reste (D79-Strang, separat vom Kontext-Lean-Strang oben):** #1 (Whisper-Filter) + #2 (Attack/Warp → `rules/combat.py`, ADR 037) **erledigt**; **#3 verworfen** (Turn-Order — Tobi egal). Offen, von Tobi „auch gut, später": **#4** `runtime.seed_session(channel)` (den 9-Schritt-`!join`-Seed bündeln), **#5** Prompt-Zusammenbau-Besitzer (⚠️ Timing cache-vs-pull **muss** erhalten bleiben — RAG pro Turn, Recap/State nur bei Mutation; berührt nicht die ADR-034-Shims), **#6** Panel-Anpin-Helfer (`_post_mic_button`/`_post_turn_order`/`_refresh_pause_panel` → ein Lifecycle-Helfer).
...
| 9 — Memory (JSON + recaps) | ADR 004 (character/state JSON) + **ADR 015** (sheet/state split, auto-combat damage) + **ADR 027** (rolling auto-recap / context handoff — recap is no longer wrap-up-only) + **ADR 037** (attack/Warp resolution → pure `rules/combat.py`) |
