...
- **`!sprechmodus [stream|nahtlos] [flach|intoniert]`** (Aliase `!sprache`/`!voicemode`) schaltet live um, zeigt den
  ...
  Fallback + Helfer-Mapping). Suite **316 grün**, Import-Smoke im venv ok.
- _Live offen: A/B in `circlejerk` — `!sprechmodus` umschalten, je ein paar Turns + `!intro` in jedem Modus hören;
  CPU-Wartezeit bei `nahtlos` beachten. Echter Speed-Fix für „nahtlos überall" bleibt die GPU-Auslagerung (ADR 002)._

_Ältere `## Last session`-Einträge (D67 Shutdown-Leave-Limit, D66 CPU-Ursache/Schnellstart-`!intro`, D65 Seamless-Spur, D64 2000-Zeichen-Fix
u. a.): siehe **[docs/progress-archive.md](docs/progress-archive.md)**._
